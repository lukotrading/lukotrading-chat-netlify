...
exports.handler = async (event) => {
  ...
  let kb = [];
  try {
    const kbPath = path.join(__dirname, "..", "..", "knowledge", "products.json");
    const items = JSON.parse(fs.readFileSync(kbPath, "utf-8"));
    const last = [...messages].reverse().find(m=>m.role==="user")?.content?.toLowerCase() || "";
    const words = last.split(/\W+/).filter(Boolean);
    kb = items
      .map(it => {
        const keys = [it.name, ...(it.aliases||[]), it.usage, it.safety, it.notes, ...(it.sector||[])].join(" ").toLowerCase();
        const score = words.reduce((s,w)=>s+(keys.includes(w)?1:0),0);
        return { score, it };
      })
      .sort((a,b)=>b.score-a.score).slice(0,3)
      .filter(x=>x.score>0).map(x=>x.it);
  } catch {}
  ...
  try {
    ...
    if (!resp.ok) return { statusCode: 500, body: "OpenAI error: " + text };
